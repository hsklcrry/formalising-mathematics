...
begin
  split,
  {
    intros h,
    split,
    apply mem_sets_of_superset h, by exact inter_subset_left S T,
    apply mem_sets_of_superset h, by exact inter_subset_right S T,
  },
  {
    rintro ⟨h1,h2⟩,
...
begin
  split,
  {
    ...
    rw le_def at h,
    specialize h T (mem_principal_self _),
    assumption,    
  },
  {
    intro h,
...
begin
  ...
  {
    intro h,
    specialize h S (by {exact mem_principal_self _}),
    assumption
  },
  {
    intros hSF T hT,
    apply mem_sets_of_superset hSF,
    exact hT,
  }
end
...
{ sets := {X : set L | ∃ x : L, ∀ y, x ≤ y → y ∈ X},
  ...
  inter_sets := begin
    intros x y hx hy,
    cases hx with lx hlx,
    cases hy with ly hly,
    use max lx ly,
    intros l hl,
    split,
    apply hlx, exact le_of_max_le_left hl,
    apply hly, exact le_of_max_le_right hl,
  end }
